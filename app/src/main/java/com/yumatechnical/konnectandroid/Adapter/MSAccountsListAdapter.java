...
public class MSAccountsListAdapter extends ArrayAdapter<IAccount> implements Filterable {

	private List<IAccount> my_data;
	...
	private static final String TAG = MSAccountsListAdapter.class.getSimpleName();
	private LayoutInflater layoutInflater;

	public interface OnClickListener {
//		void OnSelectItem(int connectionID);
//		void OnSelectItem(String name);
		void OnSelectItem(IAccount item);
	}
	private final OnClickListener listener;


	public MSAccountsListAdapter(@NonNull Context context, int resource, @NonNull List<IAccount> objects,
...
//		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		IAccount entry = getItem(position);
		if (entry != null) {
			/* TODO fix for IAccount */
			/*if (entry.getIconBeforeText() != null) {(*/
				...
				if (entry.getUsername() != null) {
					holder.textView2.setText(entry.getUsername());
...
/*			holder.line.setPadding(entry.getLeftPadding(), entry.getTopPadding(), 0, entry.getBotPadding());
		...
		convertView.setOnClickListener(v -> {
			...
			IAccount clickedItem = my_data.get(position);
...
*/
	...
	@Nullable
	@Override
	public IAccount getItem(int position) {
...
*/
	...
	public void setData(ArrayList<IAccount> data) {
		my_data = data;
	}
/*
	@NonNull
